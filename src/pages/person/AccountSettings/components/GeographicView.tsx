...
@connect(
  ({
    ...
    const { province, city } = personAndAccountSettings;
    return {
      province,
      city,
      loading: loading.models.personAndAccountSettings,
    };
  },
)
class GeographicView extends Component<GeographicViewProps> {
  componentDidMount = () => {
    ...
    if (dispatch) {
      dispatch({
        type: 'personAndAccountSettings/fetchProvince',
      });
    }
  };
  ...
  componentDidUpdate(props: GeographicViewProps) {
    ...
    if (!props.value && !!value && !!value.province) {
      if (dispatch) {
        dispatch({
          type: 'personAndAccountSettings/fetchCity',
