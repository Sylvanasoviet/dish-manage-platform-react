import { IConfig, IPlugin } from 'umi-types';
import defaultSettings from './defaultSettings'; // https://umijs.org/config/

import slash from 'slash2';
import themePluginConfig from './themePluginConfig';
const { pwa } = defaultSettings; // preview.pro.ant.design only do not use in your production ;
// preview.pro.ant.design 专用环境变量，请不要在你的项目中使用它。

const { ANT_DESIGN_PRO_ONLY_DO_NOT_USE_IN_YOUR_PRODUCTION } = process.env;
const isAntDesignProPreview = ANT_DESIGN_PRO_ONLY_DO_NOT_USE_IN_YOUR_PRODUCTION === 'site';
const plugins: IPlugin[] = [
  [
    'umi-plugin-react',
    {
      antd: true,
      dva: {
        hmr: true,
      },
      locale: {
        // default false
        enable: true,
        // default zh-CN
        default: 'zh-CN',
        // default true, when it is true, will use `navigator.language` overwrite default
        baseNavigator: true,
      },
      dynamicImport: {
        loadingComponent: './components/PageLoading/index',
        webpackChunkName: true,
        level: 3,
      },
      pwa: pwa
        ? {
            workboxPluginMode: 'InjectManifest',
            workboxOptions: {
              importWorkboxFrom: 'local',
            },
          }
        : false, // default close dll, because issue https://github.com/ant-design/ant-design-pro/issues/4665
      // dll features https://webpack.js.org/plugins/dll-plugin/
      // dll: {
      //   include: ['dva', 'dva/router', 'dva/saga', 'dva/fetch'],
      //   exclude: ['@babel/runtime', 'netlify-lambda'],
      // },
    },
  ],
  [
    'umi-plugin-pro-block',
    {
      moveMock: false,
      moveService: false,
      modifyRequest: true,
      autoAddMenu: true,
    },
  ],
];

if (isAntDesignProPreview) {
  // 针对 preview.pro.ant.design 的 GA 统计代码
  plugins.push([
    'umi-plugin-ga',
    {
      code: 'UA-72788897-6',
    },
  ]);
  plugins.push(['umi-plugin-antd-theme', themePluginConfig]);
}

export default {
  plugins,
  hash: true,
  targets: {
    ie: 11,
  },
  // umi routes: https://umijs.org/zh/guide/router.html
  routes: [
    {
      path: '/user',
      component: '../layouts/UserLayout',
      routes: [
        {
          name: 'login',
          path: '/user/login',
          component: './user/login',
        },
        {
          name: 'result',
          icon: 'smile',
          path: '/user/userregisterresult',
          component: './user/UserRegisterResult',
        },
        {
          name: 'register',
          icon: 'smile',
          path: '/user/userregister',
          component: './user/UserRegister',
        },
      ],
    },
    {
      path: '/',
      component: '../layouts/SecurityLayout',
      routes: [
        {
          path: '/',
          component: '../layouts/BasicLayout',
          authority: ['admin', 'user'],
          routes: [
            {
              path: '/',
              redirect: '/welcome',
            },
            {
              path: '/welcome',
              name: 'welcome',
              icon: 'smile',
              component: './Welcome',
            },
            {
              path: '/admin',
              name: 'admin',
              icon: 'crown',
              component: './Admin',
              authority: ['admin'],
            },
            {
              path: '/dishes',
              name: 'dishes',
              icon: 'dashboard',
              routes: [
                {
                  name: 'arrange',
                  icon: 'smile',
                  path: '/dishes/arrange',
                  component: './dishes/Arrange',
                },
                {
                  name: 'manage',
                  icon: 'smile',
                  path: '/dishes/manage',
                  component: './dishes/Manage',
                },
                {
                  name: 'show',
                  icon: 'smile',
                  path: '/dishes/show',
                  component: './dishes/Show',
                },
              ],
            },
            {
              path: '/dashboard',
              name: 'dashboard',
              icon: 'dashboard',
              routes: [
                {
                  name: 'analysis',
                  icon: 'smile',
                  path: '/dashboard/dashboardanalysis',
                  component: './dashboard/DashboardAnalysis',
                  exact: true,
                },
                {
                  name: 'monitor',
                  icon: 'smile',
                  path: '/dashboard/dashboardmonitor',
                  component: './dashboard/DashboardMonitor',
                  exact: true,
                },
                {
                  name: 'workplace',
                  icon: 'smile',
                  path: '/dashboard/dashboardworkplace',
                  component: './dashboard/DashboardWorkplace',
                  exact: true,
                },
              ],
            },
            {
              path: '/person',
              name: 'account',
              icon: 'user',
              routes: [
                {
                  name: 'center',
                  icon: 'smile',
                  path: '/person/accountcenter',
                  component: './person/AccountCenter',
                  exact: true,
                },
                {
                  name: 'settings',
                  icon: 'smile',
                  path: '/person/accountsettings',
                  component: './person/AccountSettings',
                },
              ],
            },
            {
              path: '/form',
              name: 'form',
              icon: 'form',
              routes: [
                {
                  name: 'basic-form',
                  icon: 'smile',
                  path: '/form/formbasicform',
                  component: './form/FormBasicForm',
                },
                {
                  name: 'advanced-form',
                  icon: 'smile',
                  path: '/form/formadvancedform',
                  component: './form/FormAdvancedForm',
                },
                {
                  name: 'step-form',
                  icon: 'smile',
                  path: '/form/formstepform',
                  component: './form/FormStepForm',
                },
              ],
            },
            {
              path: '/editor',
              name: 'editor',
              icon: 'edit',
              routes: [
                {
                  name: 'flow',
                  icon: 'smile',
                  path: '/editor/flow',
                  component: './editor/EditorFlow',
                },
                {
                  name: 'koni',
                  icon: 'smile',
                  path: '/editor/koni',
                  component: './editor/EditorKoni',
                },
                {
                  name: 'mind',
                  icon: 'smile',
                  path: '/editor/mind',
                  component: './editor/EditorMind',
                },
              ],
            },
            {
              path: '/list',
              name: 'list',
              icon: 'table',
              routes: [
                {
                  name: 'basic-list',
                  icon: 'smile',
                  path: '/list/listbasiclist',
                  component: './list/ListBasicList',
                },
                {
                  name: 'card-list',
                  icon: 'smile',
                  path: '/list/listcardlist',
                  component: './list/ListCardList',
                },
                {
                  name: 'search-list',
                  icon: 'smile',
                  path: '/list/listsearchapplications',
                  component: './list/ListSearchApplications',
                },
                {
                  name: 'search-list',
                  icon: 'smile',
                  path: '/list/listsearcharticles',
                  component: './list/ListSearchArticles',
                },
                {
                  name: 'search-list',
                  icon: 'smile',
                  path: '/list/listsearchprojects',
                  component: './list/ListSearchProjects',
                },
                {
                  name: 'search-list',
                  icon: 'smile',
                  path: '/list/listsearch',
                  component: './list/ListSearch',
                },
                {
                  name: 'table-list',
                  icon: 'smile',
                  path: '/list/listtablelist',
                  component: './list/ListTableList',
                },
              ],
            },
            {
              component: './404',
            },
          ],
        },
        {
          component: './404',
        },
      ],
    },
    {
      component: './404',
    },
  ],
  // Theme for antd: https://ant.design/docs/react/customize-theme-cn
  theme: {
    // ...darkTheme,
  },
  define: {
    ANT_DESIGN_PRO_ONLY_DO_NOT_USE_IN_YOUR_PRODUCTION:
      ANT_DESIGN_PRO_ONLY_DO_NOT_USE_IN_YOUR_PRODUCTION || '', // preview.pro.ant.design only do not use in your production ; preview.pro.ant.design 专用环境变量，请不要在你的项目中使用它。
  },
  ignoreMomentLocale: true,
  lessLoaderOptions: {
    javascriptEnabled: true,
  },
  disableRedirectHoist: true,
  cssLoaderOptions: {
    modules: true,
    getLocalIdent: (
      context: {
        resourcePath: string;
      },
      _: string,
      localName: string
    ) => {
      if (
        context.resourcePath.includes('node_modules') ||
        context.resourcePath.includes('ant.design.pro.less') ||
        context.resourcePath.includes('global.less')
      ) {
        return localName;
      }

      const match = context.resourcePath.match(/src(.*)/);

      if (match && match[1]) {
        const antdProPath = match[1].replace('.less', '');
        const arr = slash(antdProPath)
          .split('/')
          .map((a: string) => a.replace(/([A-Z])/g, '-$1'))
          .map((a: string) => a.toLowerCase());
        return `antd-pro${arr.join('-')}-${localName}`.replace(/--/g, '-');
      }

      return localName;
    },
  },
  manifest: {
    basePath: '/',
  }, // chainWebpack: webpackPlugin,
  // proxy: {
  //   '/server/api/': {
  //     target: 'https://preview.pro.ant.design/',
  //     changeOrigin: true,
  //     pathRewrite: { '^/server': '' },
  //   },
  // },
} as IConfig;
